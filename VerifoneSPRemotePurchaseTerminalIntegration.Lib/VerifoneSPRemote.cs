...
namespace VerifoneSPRemotePurchaseTerminalIntegration.Lib
{
    public class VerifoneSPRemote
    {
        ...
        public VerifoneSPRemote(string serverIp, int port, NLog.Logger logger)
        {
        ...
        public Result Purchase(string transactionId, string amount, bool printReceiptOnPOS = true)
        {
            ...
            if (messageStatusCode == StatusCode.OKCommand)
            {
                try
                {
                    ...
                    ParsePurchaseResponse(printReceiptOnPOS, purchaseResult, message);
                }
                catch (Exception ex)
                {
                    logger.Error(ex, _infoErrorParsingMessage, message);
        ...
        public static PurchaseResult ParsePurchaseResponse(bool printReceiptOnPOS, PurchaseResult purchaseResult, string message)
        {
            DateTime.TryParseExact(
                $"{message.Substring(18, 8)} {message.Substring(107, 6)}",
                _dateTimeFormatOnPOS,
